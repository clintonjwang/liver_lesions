...
def pretrain_cnn(trained_model, padding=['same','same'], pool_sizes=[2, 2],
	activation_type='relu', f=[128,128,128,128], kernel_size=(3,3,2), dense_units=100, skip_con=False,
	...
	#else:
	#x = layers.MaxPooling3D(pool_sizes[0])(x)
	x = layers.Conv3D(filters=f[0], kernel_size=kernel_size, padding=padding[0], trainable=False)(img)

...
def feature_id_bulk(model_nums, model_prefix='fixZ_std_', prefix=""):#noaug_
	...
	for model_ix in model_nums:
		...
		all_dense_path = join(C.model_dir, prefix+"all_dense_%d.bin" % model_ix)
		if exists(all_dense_path):
			all_dense = hf.pickle_load(all_dense_path)
		else:
			all_dense = get_overall_activations(z_train_cls, model_fc)#, samples=500)
			hf.pickle_dump(all_dense, all_dense_path)

		feature_dense_path = join(C.model_dir, prefix+"feature_dense_%d.bin" % model_ix)
		if exists(feature_dense_path):
			feature_dense = hf.pickle_load(feature_dense_path)
		else:
			feature_dense = get_feature_activations(Z_features, all_features, model_fc)
			hf.pickle_dump(feature_dense, feature_dense_path)

		train_df_path = join(C.model_dir, prefix+"train_df_%d.bin" % model_ix)
		if exists(train_df_path):
			train_df = hf.pickle_load(train_df_path)
		else:
			train_df = predict_test_features(fullM, model_fc, all_dense, feature_dense, x_train, z_train, Z_features=Z_features, num_samples=1) #2
			hf.pickle_dump(train_df, train_df_path)

		test_df = predict_test_features(fullM, model_fc, all_dense, feature_dense, x_test, z_test, num_samples=1) #30
		hf.pickle_dump(test_df, join(C.model_dir, prefix+"test_df_%d.bin" % model_ix))
		...
		hf.pickle_dump(df, join(C.model_dir, prefix+"df_%d.bin" % model_ix))

		full_dfs.append(df)

	return full_dfs

def process_feat_id_dfs(all_features, DFs, class_condition=False):
	num_features = len(all_features) # number of features
	data_dir = join(C.base_dir, 'excel')
	answer_key = join(data_dir, "ground_truth.xlsx")
	answer_key = pd.read_excel(answer_key, index_col=0)

	features_by_cls, _ = collect_features()
	...
	for df in DFs:
		...
		for f in sorted(all_features):
			...
			for key, row in answer_key.iterrows():
				...
				if class_condition:
					pred_cls = df.loc[key, 'pred_cls']
					pred_features = [f for f in pred_features if f in features_by_cls[pred_cls]]
		...
		for key, row in answer_key.iterrows():
			...
			ignore_features = row[['ignore_1', 'ignore_2']].dropna().values
			pred_features = df.loc[key][['feature_1', 'feature_2', 'feature_3', 'feature_4']].dropna().values
			if len(set([f for f in answer_features if f in pred_features]).symmetric_difference([f \
				for f in pred_features if f not in ignore_features])) == 0:
				n+=1
		perfect.append(n)
		...
		miscls_accnums.append([accnum for accnum in answer_key.index if df.loc[accnum, 'true_cls'] != df.loc[accnum, 'pred_cls']])
	...
	return feat_df, cls_df, MISCLS, prec_history, recall_history, perfect, miscls_accnums
...
def calculate_W(f_conv_ch, all_conv_ch, all_features, relevant_features, num_rel_f, num_channels, D=(16,16,8)):
	"""gauss = np.ones(D)#get_gaussian_mask(1)
	...
	for ch_ix in range(num_channels):
		MEAN = np.mean([f_conv_ch[all_features[f_ix]][...,ch_ix].mean() for f_ix in relevant_features])
		...
		for i, f_ix in enumerate(relevant_features):
			f = all_features[f_ix]
			W[i,ch_ix] = (f_conv_ch[f][...,ch_ix].mean() - MEAN)# - all_conv_ch[...,ch_ix].mean())
...
def get_gaussian_mask(divisor=3):
	...
	gauss = np.transpose(np.tile(gauss, (8,1,1)), (1,2,0))
...
def predict_test_features(full_model, model_fc, all_neurons, feature_neurons, x_test, z_test, Z_features=None, priors=None, models_conv=None, num_samples=15):
	...
	for cls in C.cls_names:
		...
		lesion_ids[cls] = [x[:-4] for x in os.listdir(C.unaug_dir) if x[:x.find('_')] in accnums]
	...
	for img_ix in range(len(z_test)):
		...
		for aug_id in range(num_samples):
			if num_samples == 1:
				try:
					img = np.load(join(C.unaug_dir, "%s.npy" % z))
				except Exception as e:
					print(join(C.unaug_dir, "%s.npy" % z), e)
			else:
				try:
					img = np.load(join(C.aug_dir, "%s_%d.npy" % (z, aug_id)))
				except Exception as e:
					print(join(C.aug_dir, "%s_%d.npy" % (z, aug_id)), e)
		...
		for x_ix in range(test_dense.shape[0]):
			pH = kde_rodeo_local(test_dense[x_ix], all_neurons)

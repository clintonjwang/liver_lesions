...
def compute_patch_indices(img_shape, patch_size, overlap, start=None):
    if isinstance(overlap, int):
        overlap = np.asarray([overlap] * len(img_shape))
    if start is None:
        n_patches = np.ceil(img_shape / (patch_size - overlap))
        overflow = (patch_size - overlap) * n_patches - img_shape + overlap
        start = -np.ceil(overflow/2)
    elif isinstance(start, int):
        start = np.asarray([start] * len(img_shape))
    stop = img_shape + start
...
def get_random_patch_index(img_shape, patch_shape):
    """
    ...
    """
    return get_random_nd_index(np.subtract(img_shape, patch_shape))
...
def get_patch_from_3d_data(img, patch_shape, patch_index):
    """
    Returns a patch from a numpy array.
    :param img: numpy array from which to get the patch.
    :param patch_shape: shape/size of the patch.
    :param patch_index: corner index of the patch.
    :return: numpy array take from the img with the patch shape specified.
    """
    patch_index = np.asarray(patch_index, dtype=np.int16)
    patch_shape = np.asarray(patch_shape)
    img_shape = img.shape[-3:]
    if np.any(patch_index < 0) or np.any((patch_index + patch_shape) > img_shape):
        img, patch_index = fix_out_of_bound_patch_attempt(img, patch_shape, patch_index)
    return img[..., patch_index[0]:patch_index[0]+patch_shape[0], patch_index[1]:patch_index[1]+patch_shape[1],
...
def fix_out_of_bound_patch_attempt(data, patch_shape, patch_index, ndim=3):
    ...
    pad_after = np.abs(((patch_index + patch_shape) > img_shape) * ((patch_index + patch_shape) - img_shape))
...
def reconstruct_from_patches(patches, patch_indices, data_shape, default_value=0):
    ...
    for patch, index in zip(patches, patch_indices):
        ...
        if np.any(index < 0):
            ...
            index[index < 0] = 0
        if np.any((index + image_patch_shape) >= img_shape):
            fix_patch = np.asarray(image_patch_shape - (((index + image_patch_shape) >= img_shape)
                                                        * ((index + image_patch_shape) - img_shape)), dtype=np.int)
            patch = patch[..., :fix_patch[0], :fix_patch[1], :fix_patch[2]]
